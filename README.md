...
Every brief is stamped in its header (and the Markdown) with a generation time
and a **build id** — the short git commit that produced it:

```
Generated August 14, 2026 at 6:00 AM EDT · build 6ad2bd6 · EPS-wide federal market awareness
```

If a story looks wrong, check the build id against the latest commit on the
branch before anything else — an old build id means you are looking at a brief
generated by older code, and the fix is simply to regenerate (`git pull` +
re-run locally, or re-run the GitHub Actions workflow) rather than to change any
config.
